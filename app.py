...
def scrape_edisto_events():
    ...
    url = remote.get("Edisto Invitational", {}).get("events")
    if not url:
        print("❌ No URL for Edisto events in remote settings.")
        return []
    ...
    return events
#scrape bigrock events 
BIGROCK_EVENTS_CACHE = {"last_time": 0, "data": []}

def normalize_boat_name(name):
    return name.strip().lower().replace(' ', '_').replace('-', '_')

def scrape_bigrock_events():
    remote = load_remote_settings()
    url = remote.get("Big Rock", {}).get("events")
    if not url:
        print("❌ No URL for Big Rock events in remote settings.")
        return []

    now = time.time()
    if BIGROCK_EVENTS_CACHE["data"] and now - BIGROCK_EVENTS_CACHE["last_time"] < 300:
        return BIGROCK_EVENTS_CACHE["data"]
    ...
    try:
        with sync_playwright() as p:
            ...
            print(f"✅ Found {len(feed_items)} Big Rock activity items")
        ...
        print(f"❌ Big Rock scrape failed: {e}")

    BIGROCK_EVENTS_CACHE["data"] = events
    BIGROCK_EVENTS_CACHE["last_time"] = now
    return events

# ---- Run Test ----
if __name__ == "__main__":
    results = scrape_bigrock_events()
...
def scrape_bigrock_participants():
    ...
    boats = []
    tournament = "big_rock_2025"

    settings = load_remote_settings()
    url = settings.get("Big Rock", {}).get("participants")
    if not url:
        print("❌ No Big Rock participant URL in remote settings.")
        return []
    ...
    try:
        with sync_playwright() as p:
            ...
            entries = page.evaluate("""...""")  # Leave as-is
            browser.close()
...
def events():
    settings = load_settings()
    tournament_full = settings.get("tournament", "")
    tournament = "edisto" if "edisto" in tournament_full.lower() else tournament_full.lower().replace(" ", "_")
        ...
        if os.path.exists(PARTICIPANTS_MASTER_FILE):
            with open(PARTICIPANTS_MASTER_FILE, 'r') as f:
                ...
                name_to_image = {
                    ...
                    if p['uid'].startswith(tournament)
                }
