...
def _audio_router_monitor():
    ...
    while True:
        try:
            ...
            if line:
                low = line.lower()
                # Only react to new/removed sinks or cards — NOT "change" events.
                # "change on sink-input" is generated by our own move-sink-input calls,
                # so responding to it creates a tight feedback loop that pegs the CPU
                # and triggers the Pi hardware watchdog reset.
                # Also enforce a 10-second cooldown between reactive reconciles.
                is_new_or_remove = any(k in low for k in ("new sink", "remove sink", "new card", "remove card"))
                if is_new_or_remove and now - last_tick > 10.0:
